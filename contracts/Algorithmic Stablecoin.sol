...
contract AlgorithmicStablecoin is ERC20, AccessControl, ReentrancyGuard {
    ...
    // ✅ New: Reward Boosting
    uint256 public rewardBoostMultiplier = 1e18; // 1x default
    uint256 public rewardBoostEnd; // timestamp until boost is active
    ...
    // ✅ New event for reward boost
    event RewardBoostActivated(uint256 multiplier, uint256 duration);
    ...
    function _updateRewards(address u) internal {
        if (block.timestamp > lastRewardTime && totalStaked() > 0) {
            ...
            if (rewardBoostMultiplier > 1e18 && block.timestamp <= rewardBoostEnd) {
                reward = (reward * rewardBoostMultiplier) / 1e18;
            } else if (block.timestamp > rewardBoostEnd && rewardBoostMultiplier > 1e18) {
                rewardBoostMultiplier = 1e18; // reset to normal
    ...
    // ✅ Governance-controlled reward boost
    function activateRewardBoost(uint256 multiplier, uint256 duration) external onlyRole(GOVERNANCE_ROLE) {
        require(multiplier >= 1e18, "Invalid multiplier");
        rewardBoostMultiplier = multiplier;
        rewardBoostEnd = block.timestamp + duration;
        emit RewardBoostActivated(multiplier, duration);
    }
    ...
    function setMaxSupply(uint256 _max) external onlyRole(GOVERNANCE_ROLE) { maxSupply = _max; emit MaxSupplyUpdated(_max); }
    function setTransferLimitBps(uint256 bps) external onlyRole(GOVERNANCE_ROLE) { require(bps <= 10000, "bps>10000"); transferLimitBps = bps; emit TransferLimitUpdated(bps); }
    function setTransferLimitExempt(address account, bool exempt) external onlyRole(GOVERNANCE_ROLE) { transferLimitExempt[account] = exempt; emit TransferLimitExemptToggled(account, exempt); }
